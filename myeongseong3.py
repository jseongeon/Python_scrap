...
try:
    ...
    with sync_playwright() as p:
        ...
        blog_url = "https://blog.naver.com/mspatent8008"  # 블로그 URL
        page.goto(blog_url)
        ...
        # '카테고리 전체보기' 버튼 클릭
        try:
            category_all_button = iframe.query_selector("a#category0")  # '카테고리 전체보기' 버튼
            if category_all_button:
                category_all_button.click()
                time.sleep(2)  # 카테고리 로드 대기
                print("'카테고리 전체보기' 버튼을 클릭했습니다.")
            else:
                print("'카테고리 전체보기' 버튼을 찾지 못했습니다.")
        except Exception as e:
            print(f"'카테고리 전체보기' 버튼 클릭에 실패했습니다: {e}")
            raise e
